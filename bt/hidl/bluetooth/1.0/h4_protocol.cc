...
size_t H4Protocol::Send(uint8_t type, const uint8_t* data, size_t length) {
  ...
  //bt_snoop_packet_cap(type, data, 0);

  return ret;
}

void H4Protocol::OnPacketReady() {
  //const hidl_vec<uint8_t> &event = hci_packetizer_.GetPacket();

  //bt_snoop_packet_cap(hci_packet_type_, event.data(), 1);
  ...
  switch (hci_packet_type_) {
    ...
    default:
      ...
      ALOGE("%s: Unimplemented packet type %d", __func__,static_cast<int>(hci_packet_type_));
  }
