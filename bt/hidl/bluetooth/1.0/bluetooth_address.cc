...
bool BluetoothAddress::write_local_address(char* addr_str, char* file_name){
  ...
  addr_str[kStringLength]='\0';
  const uint8_t zero_bdaddr[kBytes] = {0, 0, 0, 0, 0, 0};
  uint8_t mnt_addr[kBytes] = {0, 0, 0, 0, 0, 0};
  if ((string_to_bytes(addr_str, mnt_addr))&&(memcmp(mnt_addr, zero_bdaddr, kBytes) != 0)) {
      ALOGD("%s /mnt/vendor btmac is vaild", __func__);
  } else {
      memcpy(addr_str, address, kStringLength);
  }
