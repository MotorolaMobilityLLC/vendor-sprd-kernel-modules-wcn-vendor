...
class BluetoothDeathRecipient : public hidl_death_recipient {
 ...
 // not right when mutil hidl,bug 1143511
 //private:
 //bool has_died_;
};
...
Return<void> BluetoothHci::initialize(
  ...
  ALOGI("Enter BluetoothHci::initialize()");
  for (uint8_t retry = 0; retry < 25 ; retry++) {
    if (death_recipient_->getHasDied()) {
        ALOGI("Hci is closing ,has waited for %f s!" ,retry*0.2);
  ...
  ALOGI("Begin To BluetoothHci::initialize()");
  if (cb == nullptr) {
