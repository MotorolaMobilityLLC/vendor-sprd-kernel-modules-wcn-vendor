...
static int bt_set_nosig_send_cw(uint16_t enable, uint16_t is_le, uint16_t channel)
{
    BTD("%s", __FUNCTION__);
    ...
    buf[0] = (uint8_t)enable;
    buf[1] = (uint8_t)is_le;
    buf[2] = (uint8_t)channel;
    BTD("send hci cmd, opcode = 0x%X", NONSIG_SEND_CW);
    lib_interface->hci_cmd_send(NONSIG_SEND_CW, sizeof(buf), buf, dut_vsc_cback);
    return 0;
}

static int bt_set_modulation_send_cw(uint16_t enable, uint16_t is_le, uint16_t channel)
{
    BTD("%s", __FUNCTION__);
    ...
    buf[1] = (uint8_t)is_le;
    buf[2] = (uint8_t)channel;
    BTD("send hci cmd, opcode = 0x%X", MODULATION_SEND_CW);
    lib_interface->hci_cmd_send(MODULATION_SEND_CW, sizeof(buf), buf, dut_vsc_cback);
    return 0;
}
