...
/*
* cmd: set_nosig_send_cw
* Parameter 1: enable, Size: 1 byte, 0: disable, 1: enable;
* Parameter 2: Mode, Size: 1 byte, 0: classic, 1: le;
* Parameter 3: Channel, Size: 1 byte;
*/
static int action_set_nosig_send_cw(int argc, char **argv)
{
    uint32_t data[3] = { 0 }, *p = data;
    int ret, i;
    ...
    ret = bt_test_kit->set_nosig_send_cw(enable, is_le, channel);

    if (!ret) {
        BTD("SUCCESS");
        action_result_transmit(socket_cb, "set_nosig_send_cw ok", RES_OK);
    } else {
        BTE("FAILED");
        action_result_transmit(socket_cb, "set_nosig_send_cw fail", RES_FAIL);
        ret = -1;
    }
...
/*
* cmd: set_modulation_send_cw
* Parameter 1: enable, Size: 1 byte, 0: disable, 1: enable;
* Parameter 2: Mode, Size: 1 byte, 0: classic, 1: le;
* Parameter 3: Channel, Size: 1 byte;
*/
static int action_set_modulation_send_cw(int argc, char **argv)
{
    uint32_t data[3] = { 0 }, *p = data;
    int ret, i;
    ...
    ret = bt_test_kit->set_modulation_send_cw(enable, is_le, channel);

    if (!ret) {
        BTD("SUCCESS");
        action_result_transmit(socket_cb, "set_modulation_send_cw ok", RES_OK);
    } else {
        BTE("FAILED");
        action_result_transmit(socket_cb, "set_modulation_send_cw fail", RES_FAIL);
        ret = -1;
    }
...
btif_eng_t btif_eng[] = {
    ...
    {CMD_NONSIG_SEND_CW_STR, action_set_nosig_send_cw},
    {CMD_MODULATION_SEND_CW_STR, action_set_modulation_send_cw},
