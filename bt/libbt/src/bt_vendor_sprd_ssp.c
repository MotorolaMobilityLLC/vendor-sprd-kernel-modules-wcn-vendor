...
static const char *VENDOR_LIBRARY_NAME = "libbt-ssp_bt.so";
static const char *VENDOR_LIBRARY_SYMBOL_NAME = "BLUETOOTH_VENDOR_SSP_LIB_INTERFACE";

static void *lib_handle;
static bt_vendor_ssp_interface_t *lib_interface;
...
static bool sprd_vnd_ssp_lib_open(void)
{
    BDBG("sprd_vnd_ssp_lib_open");
    if (lib_handle)
        dlclose(lib_handle);
    lib_handle = dlopen(VENDOR_LIBRARY_NAME, RTLD_NOW);
    if (!lib_handle) {
        BDBG("%s unable to open %s: %s", __func__, VENDOR_LIBRARY_NAME, dlerror());
        goto error;
    }

    lib_interface = (bt_vendor_ssp_interface_t *)dlsym(lib_handle, VENDOR_LIBRARY_SYMBOL_NAME);
    if (!lib_interface) {
        BDBG("%s unable to find symbol %s in %s: %s",
             __func__, VENDOR_LIBRARY_SYMBOL_NAME, VENDOR_LIBRARY_NAME, dlerror());
        goto error;
    }
...
static void sprd_vnd_ssp_lib_close(void) {
  BDBG("sprd_vnd_ssp_lib_close");
  if (lib_handle)
    dlclose(lib_handle);
...
void sprd_vnd_ssp_init(void)
{
    ...
    if (sprd_vnd_ssp_lib_open())
    {
        BDBG("sprd_vnd_ssp_lib_open success.");
        lib_interface->alogo_init();
        sprd_vnd_set_super_ssp_enable(1);
    }
    else
    {
        BDBG("sprd_vnd_ssp_lib_open success.");
        sprd_vnd_set_super_ssp_enable(0);
    }
}

void sprd_vnd_ssp_deinit(void)
{
    BDBG("sprd_vnd_ssp_deinit");
    sprd_vnd_ssp_lib_close();
}
