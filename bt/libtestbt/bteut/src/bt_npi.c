...
struct bt_eut_cmd bt_eut_cmds[] = {
    ...
    /* NEW ADDRESS */
    { BT_ADDRESS_NEW_REQ_INDEX, ENG_BT_NEW_ADDRESS_REQ },
    { BT_ADDRESS_NEW_INDEX, ENG_BT_NEW_ADDRESS },
...
int get_sprd_sub_str(const char *buf, char **revdata, char a, char *delim, unsigned char count,
...
static const char* dump_cmd_index(int cmd_index) {
    switch (cmd_index) {
        ...
        CASE_RETURN_STR(BT_ADDRESS_NEW_REQ_INDEX)
        CASE_RETURN_STR(BT_ADDRESS_NEW_INDEX)
        CASE_RETURN_STR(BT_RXDATA_REQ_INDEX)
        CASE_RETURN_STR(BT_TESTMODE_REQ_INDEX)
        CASE_RETURN_STR(BT_TESTMODE_INDEX)
        CASE_RETURN_STR(BT_TXPHYTYPE_REQ_INDEX)
        CASE_RETURN_STR(BT_TXPHYTYPE_INDEX)
        CASE_RETURN_STR(BT_RXPHYTYPE_REQ_INDEX)
        CASE_RETURN_STR(BT_RXPHYTYPE_INDEX)
        CASE_RETURN_STR(BT_RXMODINDEX_REQ_INDEX)
        CASE_RETURN_STR(BT_RXMODINDEXE_INDEX)
...
int cmdline_handle(char *dst, char *src)
{
    int ret = strlen(src);
...
void bt_npi_parse(int module_index, char *buf, char *rsp) {
    ...
    switch (cmd_index) {
        ...
        case BT_ADDRESS_NEW_REQ_INDEX: {
            if (strstr(cmdline, BT_GETBTADDR_ATKEY)) {
                bt_address_read(rsp);
            } else {
                bt_address_new_get(rsp);
            }
        } break;

        case BT_ADDRESS_NEW_INDEX: {
            if (strstr(cmdline, BT_SETBTADDR_ATKEY)) {
                bt_address_write(data[1], rsp);
            } else {
                bt_address_new_set(data[1], rsp);
            }
        } break;
...
int bt_address_write(const char *addr, char *rsp)
{
    char mac[2 * 6 + 5 + 1] =
    { 0 };
    int ret = 0;
    ...
    ret = sscanf(addr, "%02s%02s%02s%02s%02s%02s", mac, mac + 3, mac + 3 * 2, mac + 3 * 3,
                 mac + 3 * 4, mac + 3 * 5);
    mac[2] = mac[5] = mac[8] = mac[11] = mac[14] = ':';
    if (ret != 6)
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s", SPRDENG_ERROR, ENG_STREND);
        return 0;
    }

    fd = open("/mnt/vendor/btmac.txt", O_RDWR | O_CREAT, 0660);
    if(fd < 0)
    {
        BTE("could not open btmac.txt");
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s", SPRDENG_ERROR, ENG_STREND);
        return 0;
    }
    if (-1 == chmod("/mnt/vendor/btmac.txt", 0660))
        BTE("chmod /mnt/vendor/btmac.txt failed");
    write(fd, mac, strlen(mac));
    fsync(fd);
    close(fd);

    //no permission for engpc
    fd = open("/data/vendor/bluetooth/btmac.txt", O_RDWR | O_CREAT, 0660);
    if(fd < 0)
    {
        BTE("could not open /data/vendor/bluetooth/btmac.txt");
    }
    else
    {
        if (-1 == chmod("/data/vendor/bluetooth/btmac.txt", 0660))
            BTE("chmod /data/vendor/bluetooth/btmac.txt failed");
        write(fd, mac, strlen(mac));
        fsync(fd);
        close(fd);
    }
    ...
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s", SPRDENG_OK, ENG_STREND);
    RESPONSE_DUMP(rsp);
...
int bt_address_new_set(const char *addr, char *rsp)
{
    char mac[2 * 6 + 5 + 1] =
    { 0 };
    int ret = 0;
    ...
    fd = open("/mnt/vendor/btmac.txt", O_RDWR | O_CREAT, 0664);
    if(fd < 0)
    {
        BTE("could not open btmac.txt");
        strncpy(rsp, (BTEUT_BT_MODE_BLE == g_bt_mode ? EUT_BLE_OK : EUT_BT_OK),
    ...
    if (-1 == chmod("/mnt/vendor/btmac.txt", 0664))
        BTE("chmod /mnt/vendor/btmac.txt failed");
    write(fd, mac, strlen(mac));
    fsync(fd);
    close(fd);
...
int bt_address_read(char *rsp)
{
    ...
    if (!(fp = fopen("/mnt/vendor/btmac.txt", "r+")))
    {
        snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s", SPRDENG_ERROR, ENG_STREND);
        BTE("could not open data btmac.txt");
        return 0;
    }
    r_fread = fread(buff, sizeof(mac) - 1, 1, fp);
    BTD("bt_address_read r_fread = %d", r_fread);
    fclose(fp);

    ret = sscanf(buff, "%02s:%02s:%02s:%02s:%02s:%02s", mac, mac + 2, mac + 2 * 2, mac + 2 * 3,
           mac + 2 * 4, mac + 2 * 5);
    BTD("btmac.txt %s %s ret %d", buff, mac, ret);
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN ,"%s%s", mac, ENG_STREND);
...
int bt_address_new_get(char *rsp)
{
    FILE *fp = 0;
    int ret = 0;
    int r_fread = -1;
    ...
    BTD("btmac.txt %s %s ret %d", buff, mac, ret);
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s",
             (BTEUT_BT_MODE_BLE == g_bt_mode ? BLE_ADDRESS_NEW_REQ_RET : BT_ADDRESS_NEW_REQ_RET),
             mac);
...
int bt_chipid_get(char *rsp) {
    ...
    if (id_str_pt[0] != NULL) {
        ...
        BTD("%s id_str_pt[1]: %s", __func__, id_str_pt[1]);
    }
    snprintf(rsp, BT_EUT_COMMAND_RSP_MAX_LEN, "%s%s", BT_CHIP_REQ_RET, id_str_pt[1]);
    RESPONSE_DUMP(rsp);
    return 0;
