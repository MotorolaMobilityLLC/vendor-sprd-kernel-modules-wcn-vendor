...
static uint8_t buf_lesan_disable[2]     = {0x00, 0x01};
static uint8_t buf_inq_mode[1]          = {0x02};
static uint8_t buf_write_scan_enable[1] = {0x01};
static uint8_t buf_le_scan_para[7]      = {0x01, 0x20, 0x00, 0x40, 0x01, 0x01, 0x00};
static uint8_t buf_lesan_enable[2]      = {0x01, 0x00};
static uint8_t buf_inquiry[5]           = {0x33, 0x8b, 0x9e, 0x0a, 0x00};
...
static int bt_discovery(bool inq_status)
{
    if (lib_interface == NULL) {
        BTD("%s lib_interface is NULL", __func__);
        return 0;
    }
    if (inq_status) {
        lib_interface->hci_cmd_send(HCI_WRITE_INQUIRY_MODE, sizeof(buf_inq_mode), buf_inq_mode, NULL);
        lib_interface->hci_cmd_send(HCI_WRITE_SCAN_ENABLE , sizeof(buf_write_scan_enable), buf_write_scan_enable, NULL);
        lib_interface->hci_cmd_send(HCI_BLE_WRITE_SCAN_PARAMS , sizeof(buf_le_scan_para), buf_le_scan_para, NULL);
        lib_interface->hci_cmd_send(HCI_BLE_WRITE_SCAN_ENABLE , sizeof(buf_lesan_enable), buf_lesan_enable, NULL);
        lib_interface->hci_cmd_send(HCI_INQUIRY , sizeof(buf_inquiry), buf_inquiry, NULL);
    } else {
        lib_interface->hci_cmd_send(HCI_BLE_WRITE_SCAN_ENABLE, sizeof(buf_lesan_disable), buf_lesan_disable, NULL);
        lib_interface->hci_cmd_send(HCI_INQUIRY_CANCEL , 0, NULL, NULL);
    }
    return 0;
}

static int bt_read_scan_results(bdremote_t* bdrmt_dev)
{
    int ret = -1;
    ret = lib_interface->read_discovery_device(bdrmt_dev);
    return ret;
}
