...
int wlnpi_cmd_set_preamble(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
    ...
    *(int *)s_buf = strtol(argv[0], &err, 10);
    if (err == argv[0])
    {
        return -1;
    }
...
/********************************************************************
*   name   wlnpi_cmd_set_short_guard_interval
*   ---------------------------
*   description:
*   ----------------------------
*   para        IN/OUT      type            note
...
*
********************************************************************/
int wlnpi_cmd_set_short_guard_interval(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
    char *err;
...
/********************************************************************
*   name   wlnpi_cmd_set_burst_interval
*   ---------------------------
*   description:
*   ----------------------------
*   para        IN/OUT      type            note
...
*
********************************************************************/
int wlnpi_cmd_set_burst_interval(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
    char *err;
...
int wlnpi_show_get_rx_ok(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
    ...
    fcs_fail_count = *((unsigned int *)(r_buf+8));
	{
		cmd->priv->good_rx_frame = rx_end_count;
		cmd->priv->err_rx_frame = rx_err_end_count;
		cmd->priv->fcs_fail_frame = fcs_fail_count;
	}
    snprintf(ret_buf, WLNPI_RESULT_BUF_LEN,
			 "ret: reg value: rx_end_count=%d rx_err_end_count=%d fcs_fail_count=%d :end\n",
			 rx_end_count, rx_err_end_count, fcs_fail_count);
    ...
    printf("%s", ret_buf);
    ENG_LOG("ADL leaving %s(), rx_end_count=%d rx_err_end_count=%d fcs_fail_count=%d, return 0", __func__,
			rx_end_count, rx_err_end_count, fcs_fail_count);
...
struct wlnpi_cmd_t g_cmd_table[] =
{
...

    {
        ...
        .name  = "set_short_guard_interval",
        .help  = "set_short_gurad_interval <NUM>",
        .parse = wlnpi_cmd_set_short_guard_interval,
        .show  = wlnpi_show_only_status,
    },

    {
        ...
        .name  = "set_burst_interval",
        .help  = "set_burst_interval <NUM>",
        .parse = wlnpi_cmd_set_burst_interval,
        .show  = wlnpi_show_only_status,
    },
