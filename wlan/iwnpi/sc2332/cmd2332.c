...
int wlnpi_show_set_noise_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
    ...
    snprintf(buf, 100, "type:%d channel:%d test_period:%d value:%d",type, channel,test_period, value);
    printf("type : %d, channel : %d, test_period : %d, value : %d\n",type, channel, test_period, value);
    if(NULL != (fp = fopen(IWNPI_EXEC_TMP_FILE, "w+"))) {
        int write_cnt = 0;
...
int wlnpi_cmd_get_softap_wfa_para(struct wlnpi_cmd_t *cmd, unsigned char *r_buf,
				  int r_len)
{
	ENG_LOG("ADL entry %s(), r_len = %d", __func__, r_len);
	...
	printf("ret: %d :end\n", r_buf[0]);
	ENG_LOG("ADL leaving %s(), softap wfa pmf para = %d, return 0",
		__func__, r_buf[0]);
