 ...
 * Authors:<xianwei.zhao@spreadtrum.com>
 * Owner:
 */
#include <errno.h>
#include <stdio.h>
...
static int iwnpi_hex_dump(unsigned char *name, unsigned short nLen, unsigned char *pData,
			  unsigned short len);
...
int wlnpi_cmd_set_channel(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	char *err;
	if (2 != argc && 1 != argc) {
		ENG_LOG("%s num of set channel para not correct,argc:%d\n", __func__, argc);
		return -1;
	}
	*((unsigned int *)s_buf) = strtol(argv[0], &err, 10);
	if (err == argv[0])
...
int wlnpi_cmd_set_tx_count(int argc, char **argv, unsigned int *s_buf, int *s_len)
{
	char *err;
	if (1 != argc)
		return -1;
...
int wlnpi_show_get_channel(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	ENG_LOG("ADL leaving %s(), primary20 = %d, center=%d  return 0", __func__, ch, ch);
...
int wlnpi_show_get_tx_power(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	level_a = r_buf[0];
	level_b = r_buf[2];
	/*we just ues level_a, make it compatible with Marlin3 */
	...
	printf("ret:%d:end\n", level_a);
	ENG_LOG("ADL leaving %s(), tx_power:%d return 0", __func__, level_a);
	return 0;
}
...
int wlnpi_show_roam_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	ENG_LOG("ADL entry %s(), r_len = %d", __func__, r_len);
...
int wlnpi_show_set_wmm_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	ENG_LOG("ADL entry %s(), r_len = %d", __func__, r_len);
	return 0;
}

int wlnpi_cmd_set_noise_scan(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	char *err;
	unsigned char *tmp = s_buf;
	unsigned char type, channel, duration;

	if (3 != argc) {
		ENG_LOG("Error ! para num is : %d, required 3\n", argc);
		return -1;
	}
	/* set type 0:CCA;1:noise */
	type = strtol(argv[0], &err, 10);
	if (err == argv[0]) {
		ENG_LOG("ADL entry %s get type failed\n", __func__);
		return -1;
	}
	if (type != 0 && type != 1) {
		ENG_LOG("ADL entry %s invalid type : %d\n", __func__, type);
		return -1;
	}
	*tmp = type;
	tmp += 1;
	...
	/* set channel 1-14 */
	channel = strtol(argv[1], &err, 10);
	if (err == argv[1]) {
		ENG_LOG("ADL entry %s get channel failed\n", __func__);
		return -1;
	}
	if (channel > 14) {
		ENG_LOG("ADL entry %s invalid channel : %d\n", __func__, channel);
		return -1;
	}
	*tmp = channel;
	tmp += 1;
	...
	/* set duration 0-100 */
	duration = strtol(argv[2], &err, 10);
	if (err == argv[2]) {
		ENG_LOG("ADL entry %s get duration failed\n", __func__);
		return -1;
	}
	if (duration > 100) {
		ENG_LOG("ADL entry %s invalid duration : %d\n", __func__, duration);
		return -1;
	}
	*tmp = duration;
	*s_len = 3;

	printf("show para,len:%d type:%d channel:%d duration:%d\n", *s_len, type, channel,
	       duration);
	return 0;
}

int wlnpi_show_set_noise_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	char test_period, type, channel;
	signed char value;
	unsigned char *tmp = r_buf;
	unsigned int retcnt = 0;
	char buf[100] = { 0x0 };
	FILE *fp = NULL;
	...
	ENG_LOG("ADL entry %s(), r_len = %d", __func__, r_len);
	if (r_len < 4) {
		ENG_LOG("ADL entry %s(), r_len must equl to 4", __func__);
		return -1;
	}
	...
	snprintf(buf, 100, "type:%d channel:%d test_period:%d value:%d", type, channel, test_period,
		 value);
	printf("type : %d, channel : %d, test_period : %d, value : %d\n", type, channel,
	       test_period, value);
	if (NULL != (fp = fopen(IWNPI_EXEC_TMP_FILE, "w+"))) {
		int write_cnt = 0;

		write_cnt = fputs(buf, fp);
		ENG_LOG("ADL %s(), callED fputs %s write_cnt = %d\n", __func__, buf, write_cnt);
		fclose(fp);
	} else {
...
int wlnpi_cmd_set_sar_power(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*s_len = 3;
	...
	printf("show para : %d, %d, %d\n", s_buf[0], s_buf[1], s_buf[2]);
	ENG_LOG("show para : %d, %d, %d\n", s_buf[0], s_buf[1], s_buf[2]);
...
struct wlnpi_cmd_t g_cmd_table[] = {
	 ...
	 },
	{
	 ...
	 .show = wlnpi_show_roam_status,
	 },
	{
	 /* set wmm para */
	 .id = WLNPI_CMD_SET_WMM_PARAM,
	 .name = "set_wmm",
	 .help = "set_wmm [be|bk|vi|vo] {cwmin} {cwmax} {aifs} {txop}",
	 .parse = wlnpi_cmd_set_wmm,
	 .show = wlnpi_show_set_wmm_status,
	 },
	{
	 /* get reconnect times */
	 .id = WLNPI_CMD_GET_CONN_AP_INFO,
	...
	{
	 /* set wifi adaptive */
	 .id = WLNPI_CMD_SET_ENG_MODE,
	 .name = "set_eng_mode",
	 .help = "set_eng_mode <NUM>",
	 .parse = wlnpi_cmd_set_eng_mode,
	 .show = wlnpi_show_eng_mode_status,
	 },
	{
	 /* set noise scan */
	 .id = WLNPI_CMD_SET_NOISE_SCAN,
	 .name = "set_noise_scan",
	 .help = "set_noise_scan type channel duration",
	 .parse = wlnpi_cmd_set_noise_scan,
	 .show = wlnpi_show_set_noise_status,
	 },
	{
