...
static int nl_reply_handler(struct nl_msg *msg, void *arg)
{
	...
	nla_parse(tb_msg, WLAN_NL_ATTR_MAX, genlmsg_attrdata(gnlh, 0), genlmsg_attrlen(gnlh, 0),
		  NULL);
			    ...
			    unsigned int *r_len)
{
	...
	cb = nl_cb_alloc(NL_CB_DEFAULT); /* NL_CB_DEBUG */
	if (!cb) {
		printf("failed to allocate cb callbacks\n");
		ret = 2;
		goto out_free_msg;
	}
	s_cb = nl_cb_alloc(NL_CB_DEFAULT); /* NL_CB_DEFAULT */
	if (!s_cb) {
		printf("failed to allocate s_cb callbacks\n");
		ret = 2;
		goto out_free_cb;
	}
	genlmsg_put(msg, 0, 0, wlnpi->nl_id, 0, 0, wlnpi->nl_cmd_id, 0); /* WLAN_NL_CMD_NPI */
...
static int wlan_nl_init(wlnpi_t *wlnpi)
{
	...
	wlnpi->nl_id = genl_ctrl_resolve(wlnpi->sock, "SPRD_NL");
	wlnpi->nl_cmd_id = WLAN_NL_CMD_NPI;
...
extern int iwnpi_hex_dump(unsigned char *name, unsigned short nLen, unsigned char *pData,
			  unsigned short len);
...
int main(int argc, char **argv)
{
	...
	if (cmd->id != WLNPI_CMD_GET_CHIPID)
		...
		cmd->show(cmd, r_buf + sizeof(char), r_len - sizeof(char));
