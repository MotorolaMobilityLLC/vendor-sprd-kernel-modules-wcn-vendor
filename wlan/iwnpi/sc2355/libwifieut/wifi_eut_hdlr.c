...
static int npi_result_to_at_resp(struct wifi_eut_t *eut, char *npi_output, char *rsp)
{
	...
	if (ret == 1) {
		...
		case WIFI_EUT_CMD_GET_ANTINFO:
			ret = sscanf(output, "ret: ANTINFO=%d,%d :end", &rf_2g, &rf_5g);
			if (ret != 2) {
				ENG_LOG("wifi_eut: %s %d ret = %d\n", __func__, __LINE__, ret);
				return -1;
			}
		...
		case WIFI_EUT_CMD_GET_CHIPID:
			ret = sscanf(output, "ret: %s :end", res_chip);
			if (1 != ret) {
				ENG_LOG("wifi_eut: %s %d ret = %d\n", __func__, __LINE__, ret);
				return -1;
			}
		...
		case WIFI_EUT_CMD_GET_EFUSE:
			ret = sscanf(output, "ret: efuse:%s :end", efuse);
			if (1 != ret) {
				ENG_LOG("wifi_eut: %s %d ret = %d\n", __func__, __LINE__, ret);
				return -1;
			}
...
int wifi_mac_get_addr(char *diag_cmd, char *at_rsp)
{
	...
	ENG_LOG("wifi_eut: AT command = %s", at_cmd);
	if (strncmp(WIFI_EUT_GET_ADDR, at_cmd, WIFI_EUT_ADDR_LEN) != 0) {
		ENG_LOG("wifi_eut: keywords not match(%s)", at_cmd);
		goto err;
	}

	at_cmd += WIFI_EUT_ADDR_LEN;
...
err:
	strncpy(at_rsp, "+GETWIFIADDR:ERROR", 19);
	ENG_LOG("wifi_eut: resp = %s\n", at_rsp);
	return -1;
}
...
err:
	strncpy(at_rsp, "+SETWIFIADDR:ERROR", 19);
	ENG_LOG("wifi_eut: resp = %s\n", at_rsp);
	return -1;
}
...
void register_this_module_ext(struct eng_callback * reg, int *num)
{
	...
	*num = modules_num;
	ALOGD("file:%s, func:%s %d register complete\n", __FILE__, __func__, modules_num);
}
