...
int wlnpi_show_get_mac(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	ENG_LOG("ADL leaving %s(), mac = %02x:%02x:%02x:%02x:%02x:%02x, return 0", __func__,
...
int wlnpi_show_get_channel(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	ENG_LOG("ADL leaving %s(), primary20 = %d, center=%d  return 0", __func__, primary20,
		center);
...
int wlnpi_show_get_rx_ok(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	snprintf(ret_buf, WLNPI_RESULT_BUF_LEN,
	...
	printf("%s", ret_buf);
	ENG_LOG("ADL leaving %s(), rx_end_count=%d rx_err_end_count=%d fcs_fail_count=%d, return 0",
		__func__, rx_end_count, rx_err_end_count, fcs_faiil_count);
...
int wlnpi_cmd_get_reg(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*addr = strtol(argv[1], &err, 16);
	...
	if (NULL == argv[2] || 0 == strlen(argv[2])) {
		...
		*count = strtol(argv[2], &err, 10);
		if (err == argv[2]) {
			...
			ENG_LOG("ADL %s(), exec strtol(argv[2]) is error, set count to 1",
				__func__);
			*count = 1;
		}
	}
	...
	if (*count >= 5) {
		ENG_LOG("ADL %s(), *count is too large, *count = %d, set count to 5", __func__,
			*count);
		*count = 5;
	}
...
int wlnpi_show_get_reg(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	for (i = 0, p = 0; (i < r_len / 4) && (i < WLNPI_GET_REG_MAX_COUNT); i++) {
		...
		ret = sprintf((str + (long)p), "0x%08X\t", *((int *)(r_buf + (long)i * 4)));
		p = p + ret;
	}
...
int wlnpi_show_get_connected_ap_info(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	if (!resp_ies.connect_status) {
		...
		ENG_LOG("ADL levaling %s(), connect_status = %d", __func__,
			resp_ies.connect_status);
	...
	/* connect mode */
	printf("Mode: %s\t",
	       (resp_ies.conn_mode ? IWNPI_CONN_MODE_OTHER : IWNPI_CONN_MODE_MANAGED));
	...
	/* BSSID */
	printf("BSSID: %02X:%02X:%02X:%02X:%02X:%02X\n", resp_ies.bssid[0], resp_ies.bssid[1],
	       resp_ies.bssid[2], resp_ies.bssid[3], resp_ies.bssid[4], resp_ies.bssid[5]);
...
static char *iwnpi_bss_get_vendor_ie(const char *bss, int vendor_type)
{
	...
	while (pos + 1 < end) {
		...
		if (pos[0] == WLAN_EID_VENDOR_SPECIFIC && pos[1] >= 4
		    && vendor_type == iwnpi_get_be32(&pos[2])) {
			return (char *)pos;
		}
		pos += 2 + pos[1];
	}
...
int wlnpi_cmd_set_ar(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*s_len = 3;
	...
	ENG_LOG("ADL leaving %s(), ar_flag = %d, ar_index = %d, *s_len = %d", __func__, *ar_flag,
		*ar_index, *s_len);
...
int wlnpi_show_get_ar(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	ENG_LOG("ADL leaving %s(), ar_flag = %d, ar_index= %d, return 0", __func__, ar_flag,
		ar_index);
...
int wlnpi_cmd_tx_status(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*s_len = 2;
	...
	ENG_LOG("ADL leaving %s(), ctxt_id = %d, get_or_clear = %d", __func__, ctxt_id,
		get_or_clear);
...
int wlnpi_cmd_set_ampdu_cnt(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*((signed int *)(s_buf + 2)) = strtol(argv[2], &err, 10);
	if (*s_buf < 0 || *s_buf > 2
	    || *(s_buf + 1) < 0 || *(s_buf + 1) > 64 || *(s_buf + 2) < 0 || *(s_buf + 2) > 3) {
...
int wlnpi_show_get_pa_infor(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
...
#define PAINFO_TYPE_RX		3
	...
	int i = 0,
	    offset = 0,
	    item_num = 0, sz_wlnpi_pa_info_header_t = sizeof(struct wlnpi_pa_info_header_t);
	...
	ENG_LOG("ADL entry %s(),id=%d, %s, r_len = %d", __func__, cmd->id, cmd->name, r_len);
	...
	while ((offset + sz_wlnpi_pa_info_header_t + pa_info_header->length) <= r_len) {
		pa_info_body =
		    (unsigned int *)((char *)(pa_info_header) + sz_wlnpi_pa_info_header_t);
		item_num = 0;
...
int wlnpi_show_tx_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	int i = 0;
	...
	for (i = 0; i < 4; i++) {
		printf("color_num_sdio[%d]:%11u\r\n", i, tx_status->color_num_sdio[i]);
	}
	for (i = 0; i < 4; i++) {
		printf("color_num_mac[%d]:%12u\r\n", i, tx_status->color_num_mac[i]);
	}
	for (i = 0; i < 4; i++) {
		printf("color_num_txc[%d]:%12u\r\n", i, tx_status->color_num_txc[i]);
	}
...
int wlnpi_show_rx_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	int i = 0;
	...
	for (i = 0; i < 3; i++) {
		...
		printf("last_rxdata_snr1[%d]:%19u\r\n", i, rx_status->rx_rssi[i].last_rxdata_snr1);
		printf("last_rxdata_snr2[%d]:%19u\r\n", i, rx_status->rx_rssi[i].last_rxdata_snr2);
	...
	for (i = 0; i < 5; i++) {
		printf("req_tgrt_bu_num[%d]:%20u\r\n", i, rx_status->req_tgrt_bu_num[i]);
	}
	for (i = 0; i < 3; i++) {
		printf("rx_alloc_pkt_num[%d]:%19u\r\n", i, rx_status->rx_alloc_pkt_num[i]);
	}
...
int wlnpi_show_get_sta_lut_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
...
#define STALUT_CAP_VHT_MCS9_BW20_BIT	BIT(13)
	...
	int i = 0, offset = 0, sz_wlnpi_sta_lut_t = sizeof(struct wlnpi_sta_lut_t);
	char instance_mode_str[INSTANCE_MODE_STR_LEN] = { 0 };
	unsigned int instance_mode = 0, *slut_raw = NULL;
	...
	printf("============= Mode:%u(%s), Num Of STA:%d =============\r\n",
	       instance_mode, instance_mode_str, (r_len - offset) / sz_wlnpi_sta_lut_t);
	...
	while ((offset + sz_wlnpi_sta_lut_t) <= r_len) {
		...
		printf("      mac_addr:%02x-%02x-%02x-%02x-%02x-%02x (hex)\r\n",
		       slut->mac_addr_h[1], slut->mac_addr_h[0], slut->mac_addr_l[3],
		       slut->mac_addr_l[2], slut->mac_addr_l[1], slut->mac_addr_l[0]);
		printf("      mac_index:%u\r\n", slut->mac_index);
		printf("      state:0x%x\r\n", slut->state);
		...
		printf
		    ("        u1_stbc:%u, num_streams:%u, u1_ant_confg:%u, b_vht_beam_forming:%u\r\n",
		     slut->phy_tx_mode.u1_stbc, slut->phy_tx_mode.num_streams,
		     slut->phy_tx_mode.u1_ant_confg, slut->phy_tx_mode.b_vht_beam_forming);
		printf
		    ("        txop_sleep:%u, bandwith_oper:%u, b_msr_depart_time:%u, user_num:%u\r\n",
...
int wlnpi_show_get_mac_efuse(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	printf
	    ("ret: wifi_mac: %02x:%02x:%02x:%02x:%02x:%02x bt_mac: %02x:%02x:%02x:%02x:%02x:%02x :end\n",
	     r_buf[0], r_buf[1], r_buf[2], r_buf[3], r_buf[4], r_buf[5], r_buf[6], r_buf[7],
	     r_buf[8], r_buf[9], r_buf[10], r_buf[11]
	    );
	snprintf(iwnpi_ret_buf, WLNPI_RESULT_BUF_LEN,
		 "ret: MAC:%02x%02x%02x%02x%02x%02x,%02x%02x%02x%02x%02x%02x  :end\n", r_buf[0],
		 r_buf[1], r_buf[2], r_buf[3], r_buf[4], r_buf[5], r_buf[6], r_buf[7], r_buf[8],
		 r_buf[9], r_buf[10], r_buf[11]);
	printf("iwnpi_ret_buf is %s\n", iwnpi_ret_buf);
...
int wlnpi_cmd_set_softap_wfa_para(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	char *err;
	if (1 != argc) {
		printf("%s invalid argc : %d\n", __func__, argc);
...
int wlnpi_cmd_set_sta_wfa_para(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	char *err;
	if (1 != argc) {
		printf("%s invalid argc : %d\n", __func__, argc);
...
int wlnpi_cmd_get_sta_wfa_para(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	ENG_LOG("ADL entry %s(), r_len = %d", __func__, r_len);
	...
	printf("ret: %d :end\n", r_buf[0]);
	ENG_LOG("ADL leaving %s(), station wfa pmf para = %d, return 0", __func__, r_buf[0]);
...
int wlnpi_cmd_set_country(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	char *err;
...
struct wlnpi_cmd_t g_cmd_table[] = {
	 ...
	 },
	{
	 ...
	 .name = "set_ar",
	 .help =
	 "set_ar [flag:0|1] [index:NUM] [sgi:0|1] #set auto rate flag(on or off) and index.",
	...
	/*-----CMD ID:117-----------*/
	 ...
	 .name = "set_threshold",
	 .help =
	 "set_threshold [mode:1:sta|2:ap|4:p2p_dev|5:p2p_cli|6:p2p_go|7:ibss] [rts:] [flag:]",
	...
	/*-----CMD ID:118-----------*/
	 ...
	 .name = "set_ar_dbg",
	 .help =
	 "set_ar_dbg [id:0:retry_limt|1:update_st|3:undefine] [hiq_retry_l] [hiq_retry_s] [ampdu_retry]",
...
*
********************************************************************/
int iwnpi_hex_dump(unsigned char *name, unsigned short nLen, unsigned char *pData,
		   unsigned short len)
{
	char *str;
	int i, p, ret;
