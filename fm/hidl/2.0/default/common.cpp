...
* SCAN_STEP_100KHZ 1
*/
int COM_set_step(int fd, int step)
{
    int ret = 0;
    ret = ioctl(fd, FM_IOCTL_SET_SCAN_STEP, &step);
    if (ret < 0) {
        LOGE("%s: fail, ret = %d\n", __func__, ret);
    }
...
int COM_get_bler(int fd, int *bler)
{
    int ret = 0;
    int32_t tmp = 0;
    ...
    ret = ioctl(fd, FM_IOCTL_GET_CUR_BLER_CMD, &tmp);
    *bler = (int) tmp;
    ...
    LOGD("%s, [fd=%d] [bler=%x] [ret=%d]\n", __func__, fd, *bler, ret);

    return ret;
}

int COM_get_snr(int fd, int *snr)
{
    int ret = 0;
    int32_t tmp = 0;
    ...
    ret = ioctl(fd, FM_IOCTL_GET_SNR, &tmp);
    *snr = (int) tmp;
...
int COM_get_tune(int fd, fm_seek_criteria_parm *parm)
{
    int ret = 0;

    FMR_ASSERT(parm);

    ret = ioctl(fd, FM_IOCTL_GET_SEEK_CRITERIA, parm);

    if (ret) {
        LOGE("%s, failed\n", __func__);
    }
    LOGD("%s, [fd=%d] [ret=%d] [rssi_th=%d] [snr_th=%d] [freq_offset_th=%d] [pilot_power_th=%d] [noise_power_th=%d]\n", \
        __func__, fd, ret, parm->rssi_th, parm->snr_th, parm->freq_offset_th, parm->pilot_power_th, parm->noise_power_th);

    return ret;
}

int COM_set_tune(int fd, fm_seek_criteria_parm *parm)
{
    int ret = 0;

    FMR_ASSERT(parm);
    LOGD("%s, [fd=%d] [rssi_th=%d] [snr_th=%d] [freq_offset_th=%d] [pilot_power_th=%d] [noise_power_th=%d]\n", \
        __func__, fd, parm->rssi_th, parm->snr_th, parm->freq_offset_th, parm->pilot_power_th, parm->noise_power_th);
    ret = ioctl(fd, FM_IOCTL_SET_SEEK_CRITERIA, parm);
...
int COM_get_audio(int fd, fm_audio_threshold_parm *parm)
{
    int ret = 0;

    FMR_ASSERT(parm);

    ret = ioctl(fd, FM_IOCTL_GET_AUDIO_THRESHOLD, parm);
    LOGD("%s, [fd=%d] [hbound=%d] [lbound=%d] [power_th=%d] [phyt=%d] [snr_th=%d]\n",\
        __func__, fd, parm->hbound, parm->lbound, parm->power_th, parm->phyt, parm->snr_th);
    if (ret) {
        LOGE("%s, failed\n", __func__);
    }
...
int COM_set_audio(int fd, fm_audio_threshold_parm *parm)
{
    int ret = 0;
    ...
    LOGD("%s, [fd=%d] [hbound=%d] [lbound=%d] [power_th=%d] [phyt=%d] [snr_th=%d]\n", \
        __func__, fd, parm->hbound, parm->lbound, parm->power_th, parm->phyt, parm->snr_th);
    ret = ioctl(fd, FM_IOCTL_SET_AUDIO_THRESHOLD, parm);
...
int COM_rw_reg(int fd,fm_reg_ctl_parm *para)
{
    int ret = 0;
    FMR_ASSERT(para);
    LOGD("%s, [fd=%d] [err=%d] [addr=0x%8x] [rw_flag=%d] [val=%d]\n", \
        __func__, fd, para->err, para->addr, para->rw_flag, para->val);
    ret = ioctl(fd, FM_IOCTL_RW_REG, para);
    if (ret) {
        LOGE("%s, failed\n", __func__);
    }
    LOGD("%s, [fd=%d] [ret = %d] [err=%d] [addr=0x%8x] [rw_flag=%d] [val=%d]\n",\
        __func__, fd, ret, para->err, para->addr, para->rw_flag, para->val);
...
int COM_set_region(int fd, int region)
{
    int ret = 0;

    ret = ioctl(fd, FM_IOCTL_SET_REGION, &region);
    if (ret < 0) {
        LOGE("%s: fail, ret = %d\n", __func__, ret);
    }
...
void FM_interface_init(struct fm_cbk_tbl *cbk_tbl)
{
    ...
    cbk_tbl->get_bler = COM_get_bler;
    cbk_tbl->get_snr  = COM_get_snr;
    cbk_tbl->get_tune =COM_get_tune;
    cbk_tbl->set_tune =COM_set_tune;
    cbk_tbl->get_audio =COM_get_audio;
    cbk_tbl->set_audio =COM_set_audio;
    cbk_tbl->rw_reg  = COM_rw_reg;
    //For RDS RX.
    ...
    //fm set region
    cbk_tbl->set_region = COM_set_region;
    return;
}
